...
final class DmlAnnotator {
    ...
    DmlAnnotator(ColumnMappingRegistry registry,
...
private String rewriteInsertColumnsAndValues(String sql, Map<String, String> aliasTableMap) {
    ...
    while (st.hasNext()) {
        ...
        if (st.peekIsDoubleQuotedString()) { sb.append(st.readDoubleQuotedString()); continue; }
        if (st.peekIsMyBatisParam()) { sb.append(st.readMyBatisParam()); continue; }
        if (st.peekIsHashToken()) { sb.append(st.readHashToken()); continue; }
        if (st.peekIsCdata()) { sb.append(st.readCdata()); continue; }
        if (st.peekIsXmlTag()) { sb.append(st.readXmlTag()); continue; }
        ...
        if (st.peekWord("MERGE")) {
            ...
            if (st.peekWord("INTO")) {
                ...
                if (SqlIdentifierUtil.isIdentStart(st.peek())) {
                    ...
                    mergeIntoTable = SqlIdentifierUtil.lastPart(tableToken).toUpperCase(Locale.ROOT);
                }
                continue;
            }
            continue;
        ...
        if (st.peekWord("INSERT")) {
            ...
            if (st.peekWord("INTO")) {
                ...
                String tableToken = st.readIdentifier();
                sb.append(tableToken);
                tableName = SqlIdentifierUtil.lastPart(tableToken).toUpperCase(Locale.ROOT);
                sb.append(st.readSpaces());
            } else {
                // MERGE INSERT: INSERT ( ... )
...
private ColumnListRewrite rewriteInsertColumnListParen(String parenBlock, String tableName) {
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        String colNameOnly = SqlIdentifierUtil.lastPart(colToken).trim().toUpperCase(Locale.ROOT);
        ...
        boolean deleted = (cm != null && registry.isDeleted(cm))
                || (colNameOnly.isBlank() && baseNoComma.trim().isEmpty());
        ...
        if (deleted && emitDeletedColumnComments) {
            ...
            sb.append(pfx).append(wrapBlockComment(delComments.get(0)));
            for (int k = 1; k < delComments.size(); k++) {
                sb.append(extraCommentIndent).append(wrapBlockComment(delComments.get(k)));
            }
            continue;
        }
    ...
    while (st.hasNext()) {
        ...
        if (st.peekWord("UPDATE")) {
            ...
            if (SqlIdentifierUtil.isIdentStart(st.peek())) {
                String tableToken = st.readIdentifier();
                out.append(tableToken);
                defaultUpdateTable = SqlIdentifierUtil.lastPart(tableToken).toUpperCase(Locale.ROOT);
                continue;
            }
...
private String annotateAssignmentsChunkLeadingComma(String chunk,
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        int eq = SqlTopLevelSplitter.indexOfTopLevelEquals(baseNoComma);
        ColumnMapping cm = null;
        ...
        if (eq > 0) {
            ...
            if (ref != null) {
                lhsKeyUpper = (ref.column == null) ? null : ref.column.trim().toUpperCase(Locale.ROOT);
                cm = resolveMappingForUpdateAssignment(aliasTableMap, defaultTable, ref);
            } else {
                // 방어: 주석/잡토큰이 남아 direct-ref로 못 잡는 경우
                String key = SqlIdentifierUtil.lastPart(lhs).trim().toUpperCase(Locale.ROOT);
                lhsKeyUpper = key;
...
private static void consumeHintsAndSpaces(SqlScan st, StringBuilder out) {
    boolean progressed;
    do {
        progressed = false;
...
private List<DmlItem> splitDmlItems(String body) {
    List<String> rawItems = SqlTopLevelSplitter.splitTopLevelByComma(body);
    ...
    for (int i = 0; i < rawItems.size() - 1; i++) {
        ...
        if (!tr.comments.isEmpty()) carry.get(i + 1).addAll(tr.comments);
    }
    ...
    for (int i = 0; i < rawItems.size(); i++) {
        ...
        if (!lead.comments.isEmpty()) carry.get(i).addAll(lead.comments);
    }
...
private String annotateColumnListParenLeadingComma(String parenBlock, String tableName) {
    ...
    List<String> cols = SqlTopLevelSplitter.splitTopLevelByComma(inside);
    String tn = (tableName == null) ? "" : tableName.trim().toUpperCase(Locale.ROOT);
    ...
    for (int i = 0; i < cols.size(); i++) {
        ...
        if (cm != null && registry.isDeleted(cm)) deleted = true;
        // 이미 변환 과정에서 컬럼 토큰이 비어버린 경우(",\n\n," 같은 케이스)도 삭제로 취급
        if (colNameOnly.isBlank() && baseNoComma.trim().isEmpty()) deleted = true;

        if (deleted && emitDeletedColumnComments) {
            sb.append(prefixForDeletedInList(sb, baseNoComma, defaultIndent)).append(wrapBlockComment(registry.deletedComment(colNameOnly)));
            continue;
        }
...
private String formatParenListLeadingComma(String parenBlock) {
    ...
    List<String> vals = SqlTopLevelSplitter.splitTopLevelByComma(inside);
...
private static ColumnRef parseDirectColumnRef(String lhs) {
    ...
    if (dot > 0 && dot < t.length() - 1) {
        String a = t.substring(0, dot).trim();
        String c = t.substring(dot + 1).trim();
...
private ColumnMapping resolveMapping(Map<String, String> aliasTableMap, String tableAlias, String colUpper) {
    ...
    if (tableAlias != null && aliasTableMap != null) {
        String keyUpper = tableAlias.trim().toUpperCase(Locale.ROOT);
        ...
        if (table != null && !table.isBlank()) {
            String tu = table.trim().toUpperCase(Locale.ROOT);
