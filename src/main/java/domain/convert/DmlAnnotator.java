...
// ------------------------------------------------------------
    ...
    /**
     * MyBatis 동적 태그가 포함되어 있는지 판단한다.
     * - <if> / <choose> / <when> / <otherwise> / <foreach> / <trim> / <where> / <set> / <bind> / <include> 등
     * - CDATA(<![CDATA[ ... ]]>) 도 동적으로 취급한다.
     *
     * <p>동적 태그가 포함된 DML 리스트(컬럼리스트/VALUES/SET)는 콤마의 위치가 의미를 갖는다.
     * 이 경우 leading-comma 변환이나 정렬 로직은 동적 SQL을 깨뜨릴 수 있으므로 "원문 유지"를 위해 사용한다.
     */
    private static boolean containsMybatisDynamic(String s) {
        if (s == null || s.isEmpty()) return false;
        ...
        while (st.hasNext()) {
            ...
            if (st.peekIsCdata()) return true;
            if (st.peekIsXmlTag()) return true;
            st.read();
        }
        return false;
    }
...
// ------------------------------------------------------------
    ...
    private String rewriteInsertColumnsAndValues(String sql, Map<String, String> aliasTableMap) {
        ...
        while (st.hasNext()) {
            ...
            if (st.peekWord("INSERT")) {
                ...
                boolean dynamicInsertList = false;
                if (st.peek() == '(') {
                    String colParen = st.readParenBlock();
                    // ✅ MyBatis 동적 태그(<if>, <choose>...)가 포함된 컬럼 리스트는 구조(콤마 위치)가 의미를 갖는다.
                    //    (leading-comma 변환/정렬/삭제동기화)로 콤마가 태그 밖으로 이동하면 동적 SQL이 깨질 수 있으므로
                    //    태그가 있는 경우에는 "원문 그대로" 유지한다.
                    if (containsMybatisDynamic(colParen)) {
                        dynamicInsertList = true;
                        sb.append(colParen);
                    } else {
                ...
                if (st.peekWord("VALUES")) {
                    ...
                    if (st.peek() == '(') {
                        ...
                        // ✅ MyBatis 동적 태그가 포함된 VALUES 리스트도 콤마 위치가 의미를 갖는다.
                        //    태그가 있으면 원문을 유지하고, 태그가 없을 때만 기존 로직(leading comma/삭제동기화)을 적용한다.
                        if (dynamicInsertList || containsMybatisDynamic(valParen)) {
                            sb.append(valParen);
                        } else if (colRw != null && !colRw.deletedIndexes.isEmpty() && emitDeletedColumnComments) {
            ...
            if (st.peekWord("VALUES")) {
                ...
                if (st.peek() == '(') {
                    ...
                    // ✅ MyBatis 동적 태그가 있으면 콤마 이동이 위험하므로 그대로 유지
                    sb.append(containsMybatisDynamic(paren) ? paren : formatParenListLeadingComma(paren));
                    continue;
                }
                continue;
            }
    ...
    private String annotateAssignmentsChunkLeadingComma(String chunk,
        ...
        // ✅ MyBatis 동적 태그(<if>, <choose>...)가 포함된 SET chunk는
        //    콤마/주석 정렬이 동적 SQL 의미를 바꿀 수 있으므로 원문 유지
        if (containsMybatisDynamic(chunk)) return chunk;
