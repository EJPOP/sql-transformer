 ...
 */
public final class MybatisAsisResidueValidator {

    private final ColumnMappingRegistry registry;

    public MybatisAsisResidueValidator(ColumnMappingRegistry registry) {
    ...
    public void validate(String convertedSql,
        ...
        if (failFast) {
            throw new AsisResidueFailFastException(msg + " (" + detail + ")");
        }
    ...
    /**
     * failFast=true 일 때 AS-IS 잔존이 검출되면 이 예외로 중단한다.
     * (CLI에서는 잡아서 즉시 종료/중단 처리)
     */
