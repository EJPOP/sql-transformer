...
final class SelectRenderer {
    ...
    SelectRenderer(ColumnMappingRegistry registry,
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        if (ref != null) {
            ...
            if (alias != null && isMappableColumnIdAlias(alias)) {
                ...
                if (mapping == null) {
                    ...
                    if (mapping != null) {
                        mappingResult = new ResolveResult(mapping, ResolveStatus.FOUND, alias.trim().toUpperCase(Locale.ROOT));
                    }
                }
            } else {
        ...
        if (deleted && emitDeletedColumnComments
            ...
            if (ref != null && ref.column != null && !ref.column.isBlank()) {
                delKey = ref.column.trim().toUpperCase(Locale.ROOT);
            } else if (alias != null && !alias.isBlank()) {
                delKey = alias.trim().toUpperCase(Locale.ROOT);
            } else {
                ColumnRef inner = findFirstColumnRefInExpression(exprOnly);
                if (inner != null && inner.column != null && !inner.column.isBlank()) {
                    delKey = inner.column.trim().toUpperCase(Locale.ROOT);
                }
            }
            ...
            out.append(prefix).append(wrapBlockComment(delComments.get(0)));
        ...
        out.append(prefix).append(rendered);
...
String convertSqlFragmentToTobe(String fragment,
    ...
    SqlScan st = new SqlScan(fragment);
    while (st.hasNext()) {
...
private List<SelectItem> splitSelectItems(String selectBody) {
    List<String> rawItems = SqlTopLevelSplitter.splitTopLevelByComma(selectBody);
