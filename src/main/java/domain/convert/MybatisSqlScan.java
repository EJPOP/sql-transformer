...
final class MybatisSqlScan {
    final String s;
    int pos = 0;

    MybatisSqlScan(String s) {
        this.s = (s == null) ? "" : s;
    }
    ...
    // - Some older converters used MybatisSqlScan#has(), #peekIsSingleQuote(), etc.
    //   Provide thin wrappers to keep call-sites stable.
    // ---------------------------------------------------------------------
    boolean has() {
        return hasNext();
    }

    boolean peekIsSingleQuote() {
        return peekIsSingleQuotedString();
    }

    boolean peekIsDoubleQuote() {
        return peekIsDoubleQuotedString();
    }

    String readSingleQuoted() {
        return readSingleQuotedString();
    }

    String readDoubleQuoted() {
        return readDoubleQuotedString();
    }

    boolean peekIsIdentifierStart() {
        char c = peek();
    ...
    boolean peekIsXmlTag() {
        ...
        String name = s.substring(nameStart, p)
                .toLowerCase();
    ...
    String readParenBlock() {
        ...
        int depth = 0;
        while (pos < s.length()) {
            if (peekIsLineComment()) {
                readLineComment();
                continue;
            }
            if (peekIsBlockComment()) {
                readBlockComment();
                continue;
            }
            if (peekIsSingleQuotedString()) {
                readSingleQuotedString();
                continue;
            }
            if (peekIsMyBatisParam()) {
                readMyBatisParam();
                continue;
            }
